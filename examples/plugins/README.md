...
```sh
mkdir -p ~/.drugclaw/plugins
```

2. Copy one or both example manifests:

```sh
cp examples/plugins/smoke-test.yaml ~/.drugclaw/plugins/
cp examples/plugins/context-test.yaml ~/.drugclaw/plugins/
```

If you use a custom plugin directory, set it in `drugclaw.config.yaml`:
