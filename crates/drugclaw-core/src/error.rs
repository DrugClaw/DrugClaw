...
mod tests {
    ...
    fn test_error_display_messages() {
        let e = DrugClawError::LlmApi("bad request".into());
        assert_eq!(e.to_string(), "LLM API error: bad request");

        let e = DrugClawError::RateLimited;
        assert_eq!(e.to_string(), "Rate limited, retry after backoff");

        let e = DrugClawError::ToolExecution("tool failed".into());
        assert_eq!(e.to_string(), "Tool execution error: tool failed");

        let e = DrugClawError::Config("missing key".into());
        assert_eq!(e.to_string(), "Config error: missing key");

        let e = DrugClawError::MaxIterations(25);
    ...
    fn test_error_from_io() {
        ...
        let e: DrugClawError = io_err.into();
    ...
    fn test_error_from_json() {
        ...
        let e: DrugClawError = json_err.into();
