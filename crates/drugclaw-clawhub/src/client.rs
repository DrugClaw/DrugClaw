...
impl ClawHubClient {
    ...
    pub async fn search(
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| DrugClawError::Config(format!("ClawHub request failed: {}", e)))?;
        let search_response: ApiSearchResponse = resp
            .json()
            .await
            .map_err(|e| DrugClawError::Config(format!("Failed to parse search results: {}", e)))?;
    ...
    pub async fn get_skill(&self, slug: &str) -> Result<SkillMeta, DrugClawError> {
        ...
        let resp = req
            ...
            .await
            .map_err(|e| DrugClawError::Config(format!("ClawHub request failed: {}", e)))?;
        let get_response: GetSkillResponse = resp
            .json()
            .await
            .map_err(|e| DrugClawError::Config(format!("Failed to parse skill metadata: {}", e)))?;
    ...
    pub async fn download_skill(
        ...
        let mut last_error: Option<DrugClawError> = None;
        for url in candidate_urls {
            ...
            let resp = match req.send().await {
                Ok(resp) => resp,
                Err(e) => {
                    last_error = Some(DrugClawError::Config(format!(
            ...
            let bytes = resp.bytes().await.map_err(|e| {
                DrugClawError::Config(format!("Failed to read download from {}: {}", url, e))
            })?;
            return Ok(bytes.to_vec());
        }

        Err(last_error.unwrap_or_else(|| {
            DrugClawError::Config("ClawHub download failed: no usable endpoint".into())
        }))
    }

    /// List versions for a skill
    pub async fn get_versions(&self, slug: &str) -> Result<Vec<SkillVersion>, DrugClawError> {
        ...
        let versions: Vec<SkillVersion> = resp
            .json()
            .await
            .map_err(|e| DrugClawError::Config(format!("Failed to parse versions: {}", e)))?;
        Ok(versions)
    }
}
