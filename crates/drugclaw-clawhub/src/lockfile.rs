...
pub fn read_lockfile(path: &Path) -> Result<LockFile, DrugClawError> {
    if !path.exists() {
        return Ok(LockFile {
            version: 1,
            skills: HashMap::new(),
    ...
    let content = std::fs::read_to_string(path)
        .map_err(|e| DrugClawError::Config(format!("Failed to read lockfile: {}", e)))?;
    let lock: LockFile = serde_json::from_str(&content)
        .map_err(|e| DrugClawError::Config(format!("Failed to parse lockfile: {}", e)))?;
    Ok(lock)
}

/// Write lockfile to disk
pub fn write_lockfile(path: &Path, lock: &LockFile) -> Result<(), DrugClawError> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| DrugClawError::Config(format!("Failed to create lockfile dir: {}", e)))?;
    }
    let content = serde_json::to_string_pretty(lock)
        .map_err(|e| DrugClawError::Config(format!("Failed to serialize lockfile: {}", e)))?;
    std::fs::write(path, content)
        .map_err(|e| DrugClawError::Config(format!("Failed to write lockfile: {}", e)))?;
    Ok(())
}
