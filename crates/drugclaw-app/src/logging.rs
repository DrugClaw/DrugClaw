...
use drugclaw_observability::logs::OtlpLogExporter;
...
pub const LOG_FILE_PREFIX: &str = "drugclaw-";
...
mod tests {
    ...
    fn test_dir() -> PathBuf {
        std::env::temp_dir().join(format!("drugclaw_logging_test_{}", Uuid::new_v4()))
    }

    #[test]
    fn test_parse_log_filename_time() {
        assert!(parse_log_filename_time("drugclaw-2026-02-08-10.log").is_some());
        assert!(parse_log_filename_time("drugclaw-2026-02-08.log").is_none());
    ...
    fn test_cleanup_old_logs_keeps_recent_removes_old() {
        ...
        fs::write(dir.join("drugclaw-2025-01-01-00.log"), "old").unwrap();
        fs::write(dir.join("drugclaw-2026-02-08-10.log"), "new").unwrap();
        ...
        assert!(!dir.join("drugclaw-2025-01-01-00.log").exists());
        assert!(dir.join("drugclaw-2026-02-08-10.log").exists());
    ...
    fn test_read_last_lines_from_logs() {
        let dir = test_dir();
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("drugclaw-2026-02-08-09.log"), "a\nb\n").unwrap();
        fs::write(dir.join("drugclaw-2026-02-08-10.log"), "c\nd\n").unwrap();
