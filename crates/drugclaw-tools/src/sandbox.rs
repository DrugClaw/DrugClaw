...
pub const DEFAULT_SANDBOX_IMAGE: &str = "drugclaw-drug-sandbox:latest";
...
fn is_default_sandbox_image(value: &String) -> bool {
    value == DEFAULT_SANDBOX_IMAGE
}

fn default_sandbox_container_prefix() -> String {
    "drugclaw-sandbox".to_string()
}
...
pub struct SandboxConfig {
    ...
    #[serde(
        ...
        skip_serializing_if = "is_default_sandbox_image"
    )]
    pub image: String,
...
impl DockerSandbox {
    ...
    fn network_mode_matches_config(&self, network_mode: &str) -> bool {
    ...
    fn parse_container_inspect(output: &str) -> Result<ContainerInspectState> {
        let trimmed = output.trim();
        let Some((running, network_mode)) = trimmed.split_once('|') else {
            bail!("unexpected inspect output: {trimmed}");
        };
        Ok(ContainerInspectState {
            running: running.trim() == "true",
            network_mode: network_mode.trim().to_string(),
        })
    }

    async fn inspect_container(&self, name: &str) -> Result<Option<ContainerInspectState>> {
        let out = tokio::process::Command::new(self.runtime.cli())
            ...
            .output()
            .await
            .with_context(|| format!("failed to inspect {}", self.runtime.cli()))?;
        if !out.status.success() {
            return Ok(None);
        }
        let stdout = String::from_utf8_lossy(&out.stdout);
        Ok(Some(Self::parse_container_inspect(&stdout)?))
    }
    ...
    async fn remove_container(&self, name: &str) -> Result<()> {
        let out = tokio::process::Command::new(self.runtime.cli())
            ...
            .output()
            .await
            .with_context(|| format!("failed to remove {}", self.runtime.cli()))?;
        if !out.status.success() {
            let stderr = String::from_utf8_lossy(&out.stderr);
            bail!("{} rm -f failed: {}", self.runtime.cli(), stderr.trim());
        }
        Ok(())
    }
}
...
impl Sandbox for DockerSandbox {
    ...
    async fn ensure_ready(&self, session_key: &str) -> Result<()> {
        ...
        if let Some(state) = self.inspect_container(&name).await? {
            if state.running && self.network_mode_matches_config(&state.network_mode) {
                return Ok(());
            }
            tracing::warn!(
                container = %name,
                ...
                desired_no_network = self.config.no_network,
                "recreating sandbox container to match current config"
            );
...
mod tests {
    ...
    #[test]
    fn test_sandbox_config_defaults_to_network_enabled() {
        let cfg = SandboxConfig::default();
        assert!(!cfg.no_network);
    }

    #[test]
    fn test_parse_container_inspect_state() {
        let parsed = DockerSandbox::parse_container_inspect("true|none\n").unwrap();
        assert_eq!(
            parsed,
    ...
    #[test]
    fn test_network_mode_matching_respects_no_network_policy() {
        let offline = DockerSandbox::new(
        ...
        assert!(offline.network_mode_matches_config("none"));
        assert!(!offline.network_mode_matches_config("default"));

        let online = DockerSandbox::new(
            ContainerRuntime::Docker,
            SandboxConfig {
                no_network: false,
                ..SandboxConfig::default()
        ...
        assert!(online.network_mode_matches_config("default"));
        assert!(online.network_mode_matches_config("bridge"));
        assert!(!online.network_mode_matches_config("none"));
    }
    ...
    async fn test_router_falls_back_to_host_when_runtime_missing_and_not_required() {
        ...
        let out = router.exec("chat-1", "printf drugclaw-smoke", &opts).await;
        let out = out.expect("expected host fallback execution");
        assert_eq!(out.exit_code, 0);
        assert_eq!(out.stdout, "drugclaw-smoke");
    }
