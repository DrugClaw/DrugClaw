...
impl OtlpMetricExporter {
    pub fn from_observability(observability: Option<&Value>) -> Option<Arc<Self>> {
        ...
        let provider = SdkMeterProvider::builder()
            ...
            .with_reader(reader)
            .build();
        let meter = provider.meter("drugclaw.observability.metrics");
        let instruments = MetricInstruments {
            http_requests: meter.u64_counter("drugclaw_http_requests").build(),
            llm_completions: meter.u64_counter("drugclaw_llm_completions").build(),
            llm_input_tokens: meter.u64_counter("drugclaw_llm_input_tokens").build(),
            llm_output_tokens: meter.u64_counter("drugclaw_llm_output_tokens").build(),
            tool_executions: meter.u64_counter("drugclaw_tool_executions").build(),
            mcp_calls: meter.u64_counter("drugclaw_mcp_calls").build(),
            mcp_rate_limited_rejections: meter
                .u64_counter("drugclaw_mcp_rate_limited_rejections")
                .build(),
            mcp_bulkhead_rejections: meter
                .u64_counter("drugclaw_mcp_bulkhead_rejections")
                .build(),
            mcp_circuit_open_rejections: meter
                .u64_counter("drugclaw_mcp_circuit_open_rejections")
                .build(),
            active_sessions: meter
                .i64_up_down_counter("drugclaw_active_sessions")
                .build(),
        };
        info!(
            endpoint = %endpoint,
