    ...
    pub fn load_skill_checked(&self, name: &str) -> Result<(SkillMetadata, String), String> {
        ...
        for skill in all_skills {
            ...
            if !skill.available {
                ...
                return Err(format!(
                    "Skill '{name}' is currently unavailable: {reason}\nRun `drugclaw skill available --all` for full diagnostics."
                ));
            }
...
"#;
    ...
    fn test_build_skills_catalog_empty() {
        let dir =
            std::env::temp_dir().join(format!("drugclaw_skills_test_{}", uuid::Uuid::new_v4()));
    ...
    fn test_build_skills_catalog_sorted_and_truncated() {
        let dir = std::env::temp_dir().join(format!(
            "drugclaw_skills_catalog_sorted_{}",
            uuid::Uuid::new_v4()
        ));
...
"#,
    ...
    fn test_build_skills_catalog_enters_compact_mode_when_many_skills() {
        let dir = std::env::temp_dir().join(format!(
            "drugclaw_skills_catalog_compact_mode_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_list_skills_formatted_all_includes_unavailable_reasons() {
        let dir =
            std::env::temp_dir().join(format!("drugclaw_skills_all_test_{}", uuid::Uuid::new_v4()));
...
"#,
    ...
    fn test_load_skill_checked_unavailable_has_diagnostic_hint() {
        let dir = std::env::temp_dir().join(format!(
            "drugclaw_skills_unavailable_test_{}",
            uuid::Uuid::new_v4()
        ));
...
"#;
    ...
    fn test_disable_skill_is_runtime_scoped() {
        let base_dir = std::env::temp_dir().join(format!(
            "drugclaw_skills_runtime_scoped_{}",
            uuid::Uuid::new_v4()
        ));
...
"#,
    ...
    fn test_set_enabled_missing_skill_returns_error() {
        let base_dir = std::env::temp_dir().join(format!(
            "drugclaw_skills_enable_missing_{}",
            uuid::Uuid::new_v4()
        ));
