...
use drugclaw_tools::sandbox::{SandboxExecOptions, SandboxExecResult, SandboxMode, SandboxRouter};
...
async fn execute_command_with_policy(
    ...
    match execution_policy {
        PluginExecutionPolicy::HostOnly => {
            drugclaw_tools::sandbox::exec_host_command(command, &opts).await
        }
...
mod tests {
    ...
    fn test_command_matches_first_token() {
        ...
        assert!(command_matches(" /HELLO   world", "/hello"));
        assert!(command_matches("/hello@DrugClawBot world", "/hello"));
        assert!(!command_matches("/hello-world", "/hello"));
