...
const LINUX_SERVICE_NAME: &str = "drugclaw-gateway.service";
const MAC_LABEL: &str = "ai.drugclaw.gateway";
const LOG_STDOUT_FILE: &str = "drugclaw-gateway.log";
const LOG_STDERR_FILE: &str = "drugclaw-gateway.error.log";
...
fn resolve_config_path(cwd: &Path) -> Option<PathBuf> {
    ...
    for candidate in ["drugclaw.config.yaml", "drugclaw.config.yml"] {
...
fn render_linux_unit(ctx: &ServiceContext) -> Result<String> {
    ...
    unit.push_str("[Unit]\n");
    unit.push_str("Description=DrugClaw Gateway Service\n");
...
fn audit_linux_unit(ctx: &ServiceContext, runtime: &LinuxRuntimeStatus) -> Vec<String> {
    ...
    if !content.contains(&expected_exec) {
        issues.push("Service ExecStart does not match current drugclaw binary".to_string());
    }
...
fn start_macos() -> Result<()> {
    ...
    if !plist_path.exists() {
        return Err(anyhow!(
            "Service not installed. Run: drugclaw gateway install"
        ));
    }
...
mod tests {
    ...
    fn test_ctx() -> ServiceContext {
        ...
        service_env.insert(
            "MICROCLAW_CONFIG".to_string(),
            "/tmp/DrugClaw/DrugClaw.config.yaml".to_string(),
        );

        ServiceContext {
            exe_path: PathBuf::from("/usr/local/bin/drugclaw"),
            working_dir: PathBuf::from("/tmp/drugclaw"),
            config_path: Some(PathBuf::from("/tmp/DrugClaw/DrugClaw.config.yaml")),
            runtime_logs_dir: PathBuf::from("/tmp/drugclaw/runtime/logs"),
            service_env,
        }
    }
    ...
    fn test_render_linux_unit_contains_expected_fields() {
        ...
        assert!(unit.contains("ExecStart=/usr/local/bin/drugclaw start"));
        assert!(unit.contains("Environment=MICROCLAW_GATEWAY=1"));
        assert!(unit.contains("MICROCLAW_CONFIG=/tmp/DrugClaw/DrugClaw.config.yaml"));
    }
    ...
    fn test_render_macos_plist_contains_required_fields() {
        ...
        assert!(normalized.contains("/tmp/drugclaw/runtime/logs/drugclaw-gateway.log"));
        assert!(normalized.contains("/tmp/drugclaw/runtime/logs/drugclaw-gateway.error.log"));
    }
    ...
    fn test_format_macos_launchagents_permission_hint_contains_target_path() {
        let p = Path::new("/Users/u/Library/LaunchAgents/ai.drugclaw.gateway.plist");
        let msg = format_macos_launchagents_permission_hint(p);
        assert!(msg.contains("Permission denied"));
        assert!(msg.contains("LaunchAgents"));
        assert!(msg.contains("ai.drugclaw.gateway.plist"));
    }

    #[test]
    fn test_resolve_runtime_logs_dir_fallback() {
        let dir = resolve_runtime_logs_dir(Path::new("/tmp/drugclaw"));
        assert!(
            dir.ends_with("runtime/logs") || dir.ends_with("drugclaw.data/runtime/logs"),
            "unexpected logs dir: {}",
            dir.display()
        );
    }
