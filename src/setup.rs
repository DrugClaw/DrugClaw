...
const CONFIG_BACKUP_DIR_NAME: &str = "drugclaw.config.backups";
...
fn parse_accounts_json_value(
    raw: &str,
    field_key: &str,
) -> Result<Option<serde_json::Map<String, serde_json::Value>>, DrugClawError> {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
        return Ok(None);
    }
    let parsed: serde_json::Value = serde_json::from_str(trimmed).map_err(|e| {
        DrugClawError::Config(format!("{field_key} must be valid JSON object: {e}"))
    })?;
...
fn parse_provider_presets_json_value(
    raw: &str,
    field_key: &str,
) -> Result<HashMap<String, LlmProviderProfile>, DrugClawError> {
    let trimmed = raw.trim();
    if trimmed.is_empty() {
...
fn parse_boolish(value: &str, default_if_empty: bool) -> Result<bool, DrugClawError> {
    ...
    match raw.as_str() {
        "true" | "1" | "yes" => Ok(true),
        "false" | "0" | "no" => Ok(false),
        _ => Err(DrugClawError::Config(format!(
...
fn parse_bot_count(value: &str, field_key: &str) -> Result<usize, DrugClawError> {
...
fn parse_u64_field(value: &str, field_key: &str) -> Result<u64, DrugClawError> {
    let trimmed = value.trim();
    if trimmed.is_empty() {
        return Err(DrugClawError::Config(format!(
    ...
    trimmed
        .parse::<u64>()
        .map_err(|_| DrugClawError::Config(format!("{field_key} must be a positive integer")))
}

fn parse_i64_list_field(value: &str, field_key: &str) -> Result<Vec<i64>, DrugClawError> {
    let trimmed = value.trim();
    if trimmed.is_empty() {
        return Ok(Vec::new());
    }

    let parse_item = |raw: &str| -> Result<i64, DrugClawError> {
            ...
            match item {
                serde_json::Value::Number(n) => {
                    let id = n.as_i64().ok_or_else(|| {
                        DrugClawError::Config(format!("{field_key} contains non-integer number"))
                    })?;
                    out.push(id);
                }
...
fn parse_string_list_field(value: &str) -> Result<Vec<String>, DrugClawError> {
    let trimmed = value.trim();
    if trimmed.is_empty() {
        return Ok(Vec::new());
    }
    if trimmed.starts_with('[') {
        let parsed: serde_json::Value = serde_json::from_str(trimmed).map_err(|e| {
            DrugClawError::Config(format!("invalid string list JSON array syntax: {e}"))
        })?;
        let arr = parsed.as_array().ok_or_else(|| {
            DrugClawError::Config("string list must be a JSON array when using [] syntax".into())
        })?;
        let mut out = Vec::new();
        for item in arr {
            let s = item
...
fn default_data_dir_for_setup() -> String {
    std::env::var_os("HOME")
        ...
        .map(|p| p.join(".drugclaw"))
        .unwrap_or_else(|| std::path::PathBuf::from(".drugclaw"))
        .to_string_lossy()
        .to_string()
}
...
fn expand_setup_path(path: &str) -> PathBuf {
...
fn validate_directory_target(field_name: &str, raw_value: &str) -> Result<(), DrugClawError> {
    let expanded = expand_setup_path(raw_value);
    let path = expanded.as_path();
    if path.exists() {
        let metadata = fs::metadata(path)?;
        if !metadata.is_dir() {
            return Err(DrugClawError::Config(format!(
                "{field_name} must point to a directory path"
            )));
        }
        if metadata.permissions().readonly() {
            return Err(DrugClawError::Config(format!(
                "{field_name} must be writable by the current user"
            )));
        }
        return Ok(());
    }

    let Some(existing_ancestor) = path.ancestors().find(|candidate| candidate.exists()) else {
        return Err(DrugClawError::Config(format!(
            "{field_name} does not have a resolvable parent directory"
        )));
    };
    let metadata = fs::metadata(existing_ancestor)?;
    if !metadata.is_dir() {
        return Err(DrugClawError::Config(format!(
            "{field_name} must live under an existing directory"
        )));
    }
    if metadata.permissions().readonly() {
        return Err(DrugClawError::Config(format!(
...
impl SetupApp {
    ...
    /// Load existing config values from drugclaw.config.yaml/.yml.
    fn load_existing_config() -> HashMap<String, String> {
        let yaml_path = if Path::new("./drugclaw.config.yaml").exists() {
            Some("./drugclaw.config.yaml")
        } else if Path::new("./drugclaw.config.yml").exists() {
            Some("./drugclaw.config.yml")
        } else {
            None
        };
    ...
    fn serialize_provider_preset_drafts(
        ...
        serde_json::to_string(&presets).map_err(|e| {
            DrugClawError::Config(format!("Failed to serialize provider profiles: {e}"))
        })
    }

    fn sync_provider_preset_page_field(&mut self) -> Result<(), DrugClawError> {
    ...
    fn clone_selected_provider_preset(&mut self) -> Result<Option<String>, DrugClawError> {
    ...
    ) -> Result<Vec<String>, DrugClawError> {
    ...
    fn telegram_slot_accounts_from_fields(
        &self,
    ) -> Result<serde_json::Map<String, serde_json::Value>, DrugClawError> {
        let mut out = serde_json::Map::new();
    ...
    fn validate_local(&self) -> Result<(), DrugClawError> {
        for field in &self.fields {
            if self.is_field_required(field) && field.value.trim().is_empty() {
                return Err(DrugClawError::Config(format!("{} is required", field.key)));
            }
        }
        ...
        if provider.is_empty() {
            return Err(DrugClawError::Config("LLM_PROVIDER is required".into()));
        }
        ...
        if !tz.is_empty() && !tz.eq_ignore_ascii_case("auto") {
            tz.parse::<chrono_tz::Tz>()
                .map_err(|_| DrugClawError::Config(format!("Invalid OVERRIDE_TIMEZONE: {tz}")))?;
        }
        ...
        validate_directory_target("DATA_DIR", &dir)?;
        ...
        validate_directory_target("WORKING_DIR", &workdir)?;
        ...
        if !memory_token_budget_raw.is_empty() {
            let memory_token_budget = memory_token_budget_raw.parse::<usize>().map_err(|_| {
                DrugClawError::Config("MEMORY_TOKEN_BUDGET must be a positive integer".into())
            })?;
        ...
        for key in [
            ...
            let value = raw
                .parse::<u64>()
                .map_err(|_| DrugClawError::Config(format!("{key} must be a positive integer")))?;
            if value == 0 {
                return Err(DrugClawError::Config(format!(
        ...
        if !embedding_dim_raw.is_empty() {
            let embedding_dim = embedding_dim_raw.parse::<usize>().map_err(|_| {
                DrugClawError::Config("EMBEDDING_DIM must be a positive integer".into())
            })?;
            if embedding_dim == 0 {
                return Err(DrugClawError::Config(
    ...
    fn validate_online(&self) -> Result<Vec<String>, DrugClawError> {
        ...
        .join()
        .map_err(|_| DrugClawError::Config("Validation thread panicked".into()))?
    }

    fn selected_provider_preset_validation_request(
        &self,
    ) -> Result<ProviderPresetValidationRequest, DrugClawError> {
        let Some(page) = self.provider_preset_page.as_ref() else {
            return Err(DrugClawError::Config(
    ...
    fn validate_selected_provider_preset_online(
        &self,
    ) -> Result<(String, Vec<String>), DrugClawError> {
    ...
    fn field_guidance(key: &str) -> (&'static str, &'static str) {
        match key {
            ...
            "LLM_USER_AGENT" => (
                "HTTP User-Agent for LLM requests. Empty means automatic DrugClaw/<version>.",
...
fn create_config_backup(path: &Path) -> Result<Option<String>, DrugClawError> {
    if !path.exists() {
        return Ok(None);
    }
    let file_name = path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("drugclaw.config.yaml");
...
fn save_config_yaml(
...
) -> Result<Option<String>, DrugClawError> {
    ...
    let parse_usize_or_default = |raw: String, key: &str, default: usize| {
        ...
        trimmed
            .parse::<usize>()
            .map_err(|e| DrugClawError::Config(format!("{key} must be a positive integer: {e}")))
    };
    ...
    yaml.push_str("# DrugClaw configuration\n\n");
    yaml.push_str(
    ...
    if channel_selected("telegram") || telegram_present {
        ...
        if let Some(accounts) = &telegram_accounts {
            ...
            let yaml_accounts = serde_yaml::to_value(serde_json::Value::Object(accounts.clone()))
                .map_err(|e| {
                DrugClawError::Config(format!("Failed to render Telegram multi-bot accounts: {e}"))
            })?;
    ...
    if channel_selected("discord") || discord_present {
        ...
        if let Some(accounts) = &discord_accounts {
            ...
            let yaml_accounts = serde_yaml::to_value(serde_json::Value::Object(accounts.clone()))
                .map_err(|e| {
                DrugClawError::Config(format!("Failed to render DISCORD_ACCOUNTS_JSON: {e}"))
            })?;
            append_yaml_value(&mut yaml, 6, &yaml_accounts);
    ...
    for (ch, include) in &dynamic_channel_include {
        ...
        if has_accounts {
            ...
            let yaml_accounts = serde_yaml::to_value(serde_json::Value::Object(accounts_map))
                .map_err(|e| {
                    DrugClawError::Config(format!("Failed to render {} accounts: {e}", ch.name))
                })?;
            append_yaml_value(&mut yaml, 6, &yaml_accounts);
        }
    }
        ...
        || !a2a_peers.is_empty()
    {
        ...
        if !a2a_peers.is_empty() {
            ...
            let yaml_peers = serde_yaml::to_value(&a2a_peers)
                .map_err(|e| DrugClawError::Config(format!("Failed to render A2A peers: {e}")))?;
    ...
    if provider_presets.is_empty() {
        ...
        yaml.push_str("#     llm_user_agent: \"drugclaw/1.0\"\n");
        ...
        let yaml_presets = serde_yaml::to_value(&provider_presets).map_err(|e| {
            DrugClawError::Config(format!("Failed to render provider_presets: {e}"))
        })?;
...
fn run_with_spinner<T, F>(
    ...
    label: &str,
    work: F,
) -> Result<T, DrugClawError>
where
    T: Send + 'static,
    F: FnOnce() -> Result<T, DrugClawError> + Send + 'static,
{
    let (tx, rx) = mpsc::channel::<Result<T, DrugClawError>>();
    std::thread::spawn(move || {
        let _ = tx.send(work());
    });
...
fn try_save(terminal: &mut DefaultTerminal, app: &mut SetupApp) -> Result<(), DrugClawError> {
    ...
    let backup = match run_with_spinner(
        terminal,
        app,
        "Saving (3/3): writing drugclaw.config.yaml",
        move || save_config_yaml(Path::new("drugclaw.config.yaml"), &values),
    ) {
        Ok(v) => v,
        Err(e) => {
            app.status = format!("Cannot save: {e}");
    ...
    app.status = "Saved drugclaw.config.yaml".into();
    app.completed = true;
    Ok(())
}
...
fn try_save_skip_online(
    ...
    let backup = match run_with_spinner(
        terminal,
        app,
        "Saving (2/2): writing drugclaw.config.yaml",
        move || save_config_yaml(Path::new("drugclaw.config.yaml"), &values),
    ) {
        Ok(v) => v,
        Err(e) => {
    ...
    app.status = "Saved drugclaw.config.yaml (online validation skipped)".into();
    app.completed = true;
    Ok(())
}

fn run_wizard(mut terminal: DefaultTerminal) -> Result<bool, DrugClawError> {
...
pub fn run_setup_wizard() -> Result<bool, DrugClawError> {
    enable_raw_mode()?;
...
pub fn enable_sandbox_in_config() -> Result<String, DrugClawError> {
    let Some(path) = Config::resolve_config_path()? else {
        return Err(DrugClawError::Config(
            "No drugclaw.config.yaml found. Run `drugclaw setup` first.".to_string(),
        ));
    };
...
mod tests {
    ...
    fn test_setup_loads_existing_web_hook_settings() {
        ...
        let temp = std::env::temp_dir().join(format!(
            "drugclaw_setup_load_web_hooks_{}",
        ...
        std::fs::write(
            temp.join("drugclaw.config.yaml"),
            r#"
bot_username: bot
api_key: key
channels:
...
"#,
        ...
        let _ = std::fs::remove_file(temp.join("drugclaw.config.yaml"));
...
"#,
    ...
    fn test_setup_loads_existing_provider_presets_from_legacy_llm_providers() {
        let _guard = env_lock();
        let temp = std::env::temp_dir().join(format!(
            "drugclaw_setup_load_provider_presets_{}",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        std::fs::create_dir_all(&temp).unwrap();
        let old_cwd = std::env::current_dir().unwrap();
...
"#,
    ...
    fn test_setup_loads_existing_telegram_topic_routing() {
        let _guard = env_lock();
        let temp = std::env::temp_dir().join(format!(
            "drugclaw_setup_load_telegram_topic_routing_{}",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        std::fs::create_dir_all(&temp).unwrap();
        let old_cwd = std::env::current_dir().unwrap();
...
"#,
    ...
    fn test_setup_loads_existing_subagents_settings() {
        let _guard = env_lock();
        let temp = std::env::temp_dir().join(format!(
            "drugclaw_setup_load_subagents_{}",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        std::fs::create_dir_all(&temp).unwrap();
        let old_cwd = std::env::current_dir().unwrap();
...
"#,
    ...
    fn test_save_config_yaml_writes_provider_presets_and_channel_refs() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_provider_presets_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_profile_overrides_as_provider_presets_not_models() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_provider_profile_override_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_provider_preset_serialization_keeps_user_agent_and_show_thinking() {
        let json = SetupApp::serialize_provider_preset_drafts(&[ProviderPresetDraft {
            ...
            user_agent: "drugclaw-test/1.0".into(),
        ...
        assert_eq!(preset.llm_user_agent.as_deref(), Some("drugclaw-test/1.0"));
    ...
    fn test_selected_provider_preset_validation_request_uses_current_entry_values() {
        ...
        assert_eq!(request.user_agent, "drugclaw-test/1.0");
    ...
    fn test_save_config_yaml_respects_high_risk_confirmation_toggle() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_high_risk_confirm_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_subagents_section() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_subagents_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_escapes_windows_directory_paths() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_windows_path_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        ...
        values.insert("LLM_PROVIDER".into(), "anthropic".into());
        values.insert("LLM_API_KEY".into(), "key".into());
        values.insert("DATA_DIR".into(), r#"C:\Users\alice\.drugclaw"#.into());
        values.insert(
            "WORKING_DIR".into(),
            r#"C:\Users\alice\.drugclaw\working_dir"#.into(),
        );

        save_config_yaml(&yaml_path, &values).unwrap();
        let s = fs::read_to_string(&yaml_path).unwrap();
        assert!(s.contains(r#"data_dir: "C:\\Users\\alice\\.drugclaw""#));
        assert!(s.contains(r#"working_dir: "C:\\Users\\alice\\.drugclaw\\working_dir""#));

        let cfg: crate::config::Config = serde_yaml::from_str(&s).unwrap();
        assert_eq!(cfg.data_dir, r#"C:\Users\alice\.drugclaw"#);
        assert_eq!(cfg.working_dir, r#"C:\Users\alice\.drugclaw\working_dir"#);
    ...
    fn test_save_config_yaml_keeps_unix_directory_paths_unchanged() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_unix_path_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        ...
        values.insert("LLM_PROVIDER".into(), "anthropic".into());
        values.insert("LLM_API_KEY".into(), "key".into());
        values.insert("DATA_DIR".into(), "/home/alice/.drugclaw".into());
        values.insert(
            "WORKING_DIR".into(),
            "/home/alice/.drugclaw/working_dir".into(),
        );

        save_config_yaml(&yaml_path, &values).unwrap();
        let s = fs::read_to_string(&yaml_path).unwrap();
        assert!(s.contains(r#"data_dir: "/home/alice/.drugclaw""#));
        assert!(s.contains(r#"working_dir: "/home/alice/.drugclaw/working_dir""#));

        let cfg: crate::config::Config = serde_yaml::from_str(&s).unwrap();
        assert_eq!(cfg.data_dir, "/home/alice/.drugclaw");
        assert_eq!(cfg.working_dir, "/home/alice/.drugclaw/working_dir");
    ...
    fn test_save_config_yaml_writes_web_hook_settings() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_web_hooks_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_uses_accounts_json_for_telegram_and_discord() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_accounts_json_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_enable_sandbox_in_config_updates_mode() {
        ...
        let path = std::env::temp_dir().join(format!(
            "drugclaw_setup_enable_sandbox_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_save_config_yaml_preserves_discord_token_without_enabled_channels() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_discord_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_disables_web_when_not_selected() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_web_toggle_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_keeps_telegram_disabled_with_credentials() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_telegram_disabled_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_keeps_dynamic_channel_disabled_when_not_selected() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_dynamic_skip_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_includes_dynamic_channel_when_selected() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_dynamic_include_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_feishu_topic_mode_as_bool() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_feishu_topic_mode_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_feishu_show_progress_as_bool() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_feishu_show_progress_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_writes_slack_inbound_image_max_mb_as_number() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_slack_inbound_image_max_mb_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_save_config_yaml_uses_telegram_slot_fields_when_multibot_enabled() {
        let yaml_path = std::env::temp_dir().join(format!(
            "drugclaw_setup_tg_slots_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let mut values = HashMap::new();
