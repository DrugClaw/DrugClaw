...
fn with_high_risk_approval_marker(input: &Value) -> Value {
    ...
    if let Some(obj) = approved_input.as_object_mut() {
        obj.insert(
            "__drugclaw_high_risk_approved".to_string(),
            Value::Bool(true),
        );
        return approved_input;
    }
    serde_json::json!({
        "__drugclaw_high_risk_approved": true,
        "__drugclaw_original_input": input,
    })
}
...
- Install skills from repos (`sync_skills`, `clawhub_install`, `clawhub_search`) — use these instead of manually writing SKILL.md files. Skills go in ~/.drugclaw/skills/ (or configured skills dir).
...
mod tests {
    ...
    use drugclaw_storage::db::{Database, StoredMessage};
    use serde_json::json;
    ...
    impl LlmProvider for EmptyVisibleThenNormalLlm {
        async fn send_message(
            ...
            let saw_guard = messages.iter().any(|m| match &m.content {
                drugclaw_core::llm_types::MessageContent::Text(t) => {
    ...
    impl LlmProvider for ApprovalLoopUntilSuccessfulToolLlm {
        async fn send_message(
            ...
            for msg in messages.iter().rev() {
                ...
                if let drugclaw_core::llm_types::MessageContent::Blocks(blocks) = &msg.content {
                    for block in blocks {
                        if let drugclaw_core::llm_types::ContentBlock::ToolResult {
                            content,
                            is_error,
                            ..
                        } = block
