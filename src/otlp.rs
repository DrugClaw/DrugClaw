...
mod tests {
    ...
    fn test_build_otlp_payload() {
        ...
        assert!(metrics.iter().any(|m| m.name == "drugclaw_http_requests"));
        assert!(metrics
            .iter()
            .any(|m| m.name == "drugclaw_mcp_rate_limited_rejections"));
        assert!(metrics
            .iter()
            .any(|m| m.name == "drugclaw_mcp_bulkhead_rejections"));
        assert!(metrics
            .iter()
            .any(|m| m.name == "drugclaw_mcp_circuit_open_rejections"));
    }
