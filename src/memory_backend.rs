...
use drugclaw_storage::db::{call_blocking, Database, Memory};
...
impl MemoryProviderFailure {
    ...
    fn into_error(self) -> DrugClawError {
        DrugClawError::ToolExecution(self.to_string())
    }
}
...
fn classify_memory_error(op: impl Into<String>, err: String) -> DrugClawError {
    MemoryProviderFailure::classify(op, err).into_error()
}

fn invalid_memory_payload(op: impl Into<String>, detail: impl Into<String>) -> DrugClawError {
...
impl MemoryBackend {
    ...
    pub async fn run_startup_health_check(&self) -> Result<(), DrugClawError> {
    ...
    pub async fn get_memory_by_id(&self, id: i64) -> Result<Option<Memory>, DrugClawError> {
    ...
    pub async fn archive_memory(&self, id: i64) -> Result<bool, DrugClawError> {
...
pub trait MemoryProvider: Send + Sync {
    ...
    async fn get_memory_by_id(&self, id: i64) -> Result<Option<Memory>, DrugClawError>;
    ...
    async fn archive_memory(&self, id: i64) -> Result<bool, DrugClawError>;
...
impl FallbackMemoryProvider {
    ...
    where
        FutPrimary: std::future::Future<Output = Result<T, DrugClawError>>,
        FutFallback: std::future::Future<Output = Result<T, DrugClawError>>,
    {
        match primary.await {
            Ok(value) => {
                self.stats.record_primary_success();
...
fn fallback_reason_for_error(op_name: &str, err: &DrugClawError) -> String {
    let structured = match err {
        DrugClawError::ToolExecution(message) => Some(message.as_str()),
        _ => None,
    };
    ...
    impl MemoryProvider for FakeProvider {
        ...
        async fn get_memories_for_context(
            ...
            match &self.get_context_error {
                Some(message) => Err(DrugClawError::ToolExecution(message.clone())),
        ...
        async fn archive_memory(&self, _id: i64) -> Result<bool, DrugClawError> {
            Ok(true)
        }
