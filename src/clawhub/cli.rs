...
use drugclaw_clawhub::install::InstallOptions;
use std::path::PathBuf;
use std::sync::Arc;
use tokio::time::{sleep, Duration};

/// Retry an async operation up to 3 times with brief delays
async fn retry_with_backoff<T, F, Fut>(mut operation: F) -> Result<T, DrugClawError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<Output = Result<T, DrugClawError>>,
{
    ...
    Err(last_error
        .unwrap_or_else(|| DrugClawError::Config("Unexpected error during retry".to_string())))
}

pub async fn handle_skill_cli(args: &[String], config: &Config) -> Result<(), DrugClawError> {
    let cli = match SkillCli::try_parse_from(
        ...
        {
            err.print()
                .map_err(|e| DrugClawError::Config(e.to_string()))?;
            return Ok(());
        }
        Err(err) => return Err(DrugClawError::Config(err.to_string())),
    };
    ...
    match subcommand {
            ...
            match result {
                Ok(result) => {
                    ...
                    if result.requires_restart {
                        println!("Restart DrugClaw or run /reload-skills to activate.");
                    }
                }
        ...
        None => {
            println!("Usage: drugclaw skill <command>");
            println!("\nCommands:");
