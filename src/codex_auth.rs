...
pub fn codex_auth_file_has_access_token() -> Result<bool, DrugClawError> {
...
pub fn resolve_openai_codex_auth(
...
) -> Result<CodexAuthResolved, DrugClawError> {
    if let Ok(token) = std::env::var("OPENAI_CODEX_ACCESS_TOKEN") {
    ...
    Err(DrugClawError::Config(format!(
...
pub fn qwen_oauth_file_has_access_token() -> Result<bool, DrugClawError> {
...
pub fn refresh_openai_codex_auth_if_needed() -> Result<(), DrugClawError> {
    let auth_path = default_codex_auth_path();
    if !auth_path.exists() {
        return Ok(());
    }
    ...
    std::fs::write(
        ...
        serde_json::to_string_pretty(&parsed).map_err(|e| {
            DrugClawError::Config(format!("Failed to serialize refreshed Codex auth: {e}"))
        })?,
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn test_codex_auth_file_has_access_token_accepts_openai_api_key_only() {
        ...
        let auth_dir = std::env::temp_dir().join(format!(
            "drugclaw-codex-auth-openai-key-only-{}",
    ...
    fn test_resolve_openai_codex_auth_accepts_openai_api_key_only() {
        ...
        let auth_dir = std::env::temp_dir().join(format!(
            "drugclaw-codex-auth-openai-key-reject-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_nanos()
