...
impl Tool for GlobTool {
    ...
    async fn execute(&self, input: serde_json::Value) -> ToolResult {
        ...
        if let Err(msg) = drugclaw_tools::path_guard::check_path(&resolved_base_str) {
        ...
        match glob::glob(&full_pattern) {
            Ok(paths) => {
                let mut matches: Vec<String> = paths
                    ...
                    .collect();
                matches = drugclaw_tools::path_guard::filter_paths(matches);
                matches.sort();
...
mod tests {
    ...
    async fn test_glob_finds_files() {
        let dir = std::env::temp_dir().join(format!("drugclaw_glob_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_glob_no_matches() {
        let dir = std::env::temp_dir().join(format!("drugclaw_glob2_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn test_glob_defaults_to_working_dir() {
        let root = std::env::temp_dir().join(format!("drugclaw_glob3_{}", uuid::Uuid::new_v4()));
