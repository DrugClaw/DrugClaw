...
use drugclaw_channels::channel::deliver_and_store_bot_message;
...
impl SubagentsOrchestrateTool {
    ...
    fn merge_run_artifacts(runs: &[drugclaw_storage::db::SubagentRunRecord]) -> serde_json::Value {
    ...
    async fn execute(&self, input: serde_json::Value) -> ToolResult {
        ...
        while std::time::Instant::now() < deadline {
            ...
            let rows = match call_blocking(self.db.clone(), move |db| {
                ...
                Ok::<_, drugclaw_core::error::DrugClawError>(out)
            })
            .await
            {
                Ok(v) => v,
...
mod tests {
    ...
    async fn test_sessions_spawn_requires_task() {
        ...
        let result = tool
            .execute(json!({"__drugclaw_auth": {"caller_channel":"web", "caller_chat_id": 1}}))
            .await;
