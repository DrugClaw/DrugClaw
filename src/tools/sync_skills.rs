...
impl SyncSkillsTool {
    ...
    async fn fetch_skill_content(
        ...
        for url in candidates {
            match client
                .get(&url)
                .header("User-Agent", "DrugClaw/1.0")
                .send()
                .await
            {
    ...
    fn normalize_skill_markdown(
        ...
        let description = if !get("description").trim().is_empty() {
            ...
            format!("Synced from {source_repo} skill '{skill_name}' and adapted for DrugClaw.")
        };
...
impl Tool for SyncSkillsTool {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Sync a skill from a GitHub repository into local ~/.drugclaw/skills (or configured skills dir). Use ONLY for skills from GitHub repos (owner/repo format or full GitHub URL). For ClawHub skills, use clawhub_install instead. Auto-detects the source repo from the path.".into(),
