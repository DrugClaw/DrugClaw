...
mod tests {
    ...
    async fn test_read_file_success() {
        let dir = std::env::temp_dir().join(format!("drugclaw_rf_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_read_file_with_offset_and_limit() {
        let dir = std::env::temp_dir().join(format!("drugclaw_rf2_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join("test.txt");
        std::fs::write(&file, "a\nb\nc\nd\ne").unwrap();
    ...
    async fn test_read_file_resolves_relative_to_working_dir() {
        let root = std::env::temp_dir().join(format!("drugclaw_rf3_{}", uuid::Uuid::new_v4()));
