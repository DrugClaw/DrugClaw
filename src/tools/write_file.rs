...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, input: serde_json::Value) -> ToolResult {
        ...
        if let Err(msg) = drugclaw_tools::path_guard::check_path(&resolved_path_str) {
        ...
        if resolved_path.file_name().and_then(|f| f.to_str()) == Some("SKILL.md") {
            ...
            if in_runtime_dir || !in_correct_dir {
                return ToolResult::error(format!(
                    "Wrong directory for skills! SKILL.md files MUST be written to ~/.drugclaw/skills/<skill-name>/SKILL.md (or configured skills dir) — \
...
mod tests {
    ...
    async fn test_write_file_success() {
        let dir = std::env::temp_dir().join(format!("drugclaw_wf_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_write_file_creates_parent_dirs() {
        let dir = std::env::temp_dir().join(format!("drugclaw_wf2_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_write_file_resolves_relative_to_working_dir() {
        let root = std::env::temp_dir().join(format!("drugclaw_wf3_{}", uuid::Uuid::new_v4()));
