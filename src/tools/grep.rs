    ...
    if metadata.is_file() {
        ...
        for entry in std::fs::read_dir(path)? {
            ...
            if entry_path.is_dir() {
                ...
                if drugclaw_tools::path_guard::is_blocked(&entry_path) {
                    continue;
                }
...
mod tests {
    ...
    fn setup_grep_dir() -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("drugclaw_grep_{}", uuid::Uuid::new_v4()));
    ...
    fn test_grep_file_function() {
        let dir = std::env::temp_dir().join(format!("drugclaw_gf_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_grep_recursive_skips_hidden_dirs() {
        let dir = std::env::temp_dir().join(format!("drugclaw_gr_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_grep_defaults_to_working_dir() {
        let root = std::env::temp_dir().join(format!("drugclaw_grep2_{}", uuid::Uuid::new_v4()));
