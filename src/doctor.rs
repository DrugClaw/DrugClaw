...
fn check_config(report: &mut DoctorReport) {
    match Config::resolve_config_path() {
        ...
        Ok(None) => report.push(
            ...
            "Config file",
            CheckStatus::Warn,
            "drugclaw.config.yaml not found".to_string(),
            Some("Run `drugclaw setup` to create configuration.".to_string()),
        ),
        Err(err) => report.push(
            "config.file",
            "Config file",
...
fn check_path(report: &mut DoctorReport) {
    ...
    } else if command_exists("drugclaw") {
        report.push(
            "path.drugclaw",
            "drugclaw in PATH",
            CheckStatus::Pass,
            "drugclaw is discoverable in PATH".to_string(),
            None,
        );
    } else {
        report.push(
            ...
            "drugclaw in PATH",
            CheckStatus::Warn,
            "drugclaw is not discoverable in PATH".to_string(),
            Some("Add the drugclaw binary directory to PATH.".to_string()),
        );
    }
}
...
fn check_mcp_dependencies(report: &mut DoctorReport) {
    let data_root = match Config::load() {
        ...
        Err(_) => PathBuf::from("./drugclaw.data"),
    };
...
fn check_sandbox_config(report: &mut DoctorReport) {
    let config = match Config::load() {
        ...
        Err(err) => {
            report.push(
                ...
                Some("Run `drugclaw setup` first.".to_string()),
            );
            return;
        }
    };
    ...
    report.push(
        ...
        if matches!(config.sandbox.mode, SandboxMode::Off) {
            Some("Enable quickly: `drugclaw setup --enable-sandbox`.".to_string())
        } else {
            None
        },
    );
...
fn default_mount_allowlist_path() -> Option<PathBuf> {
    user_home_dir().map(|h| h.join(".drugclaw/sandbox-mount-allowlist.txt"))
}

fn print_report(report: &DoctorReport) {
    println!("DrugClaw Doctor");
    println!(
...
mod tests {
    ...
    fn test_build_report_has_web_fetch_validation_checks() {
        let _guard = env_lock();
        let path = std::env::temp_dir().join(format!(
            "drugclaw_doctor_webfetch_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
    ...
    fn test_build_report_warns_when_feed_sync_enabled_without_sources() {
        let _guard = env_lock();
        let path = std::env::temp_dir().join(format!(
            "drugclaw_doctor_feed_sync_warn_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let mut cfg = Config::test_defaults();
    ...
    fn test_build_report_fails_invalid_feed_source_url() {
        let _guard = env_lock();
        let path = std::env::temp_dir().join(format!(
            "drugclaw_doctor_feed_sync_invalid_url_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
        let mut cfg = Config::test_defaults();
        cfg.web_fetch_url_validation.feed_sync.enabled = true;
        ...
        cfg.web_fetch_url_validation.feed_sync.sources =
            vec![drugclaw_tools::web_fetch::WebFetchFeedSource {
                enabled: true,
                mode: drugclaw_tools::web_fetch::WebFetchFeedMode::Denylist,
                url: "notaurl".to_string(),
                format: drugclaw_tools::web_fetch::WebFetchFeedFormat::Lines,
