...
# End-to-end Matrix smoke test for the DrugClaw research runtime.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORK_DIR="${MATRIX_SMOKE_DIR:-/tmp/drugclaw-matrix-smoke}"
PROJECT_NAME="${MATRIX_SMOKE_PROJECT:-drugclaw-matrix-smoke}"
...
LOG_FILE="$WORK_DIR/drugclaw.log"
CONFIG_FILE="$WORK_DIR/drugclaw.matrix-smoke.yaml"
STATE_FILE="$WORK_DIR/state.env"
RUNTIME_ROOT="$WORK_DIR/drugclaw.data"
DB_PATH="$RUNTIME_ROOT/runtime/drugclaw.db"
...
cleanup() {
  if [[ -n "${DRUGCLAW_PID:-}" ]]; then
    kill "$DRUGCLAW_PID" >/dev/null 2>&1 || true
    wait "$DRUGCLAW_PID" >/dev/null 2>&1 || true
  fi
...
ROOM_ID="$(curl -sS -X POST "$BASE_URL/_matrix/client/v3/createRoom" \
  ...
  -d '{"name":"drugclaw-matrix-smoke","preset":"private_chat","invite":["@bot:localhost"]}' | jq -r '.room_id')"
...
MICROCLAW_CONFIG="$CONFIG_FILE" cargo run --bin drugclaw -- start >"$LOG_FILE" 2>&1 &
DRUGCLAW_PID=$!
popd >/dev/null
