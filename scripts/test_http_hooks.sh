...
# Smoke-test the local HTTP hook ingress surface for the DrugClaw research runtime.
...
Examples:
  ...
  scripts/test_http_hooks.sh \
    --config api_test_drugclaw.config.yaml \
    --hooks-token my-hooks-secret

Notes:
  - Default config path: drugclaw.config.yaml
  - The script starts: cargo run -- start --config <path>
  - It validates DrugClaw hook endpoints:
    /hooks/agent, /api/hooks/agent, /hooks/wake
  - It also checks OpenClaw-compatible payload/header behavior.
EOF
}

CONFIG_PATH="drugclaw.config.yaml"
HOOKS_TOKEN=""
...
LOG_FILE="$(mktemp -t drugclaw-http-hooks-log.XXXXXX)"
cleanup() {
  if [[ -n "${MC_PID:-}" ]]; then
...
echo "[1/11] Starting DrugClaw with config: $CONFIG_PATH"
...
for _ in $(seq 1 90); do
  if ! kill -0 "$MC_PID" >/dev/null 2>&1; then
    echo "DrugClaw process exited before server became ready." >&2
    echo "---- last logs ----" >&2
...
post_json() {
  ...
  case "$auth_mode" in
    ...
    openclaw)
      ...
      ;;
    drugclaw)
      headers+=("-H" "x-drugclaw-hook-token: $HOOKS_TOKEN")
      ;;
    none)
      ;;
    *)
...
wait_for_run_id drugclaw "$BASE_URL/hooks/agent" \
  '{"message":"header drugclaw test","name":"api-test"}' >/dev/null
