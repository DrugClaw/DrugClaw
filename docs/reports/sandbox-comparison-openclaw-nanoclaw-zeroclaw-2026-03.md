...
| DrugClaw (before this PR) | Docker-only container runtime wiring | `auto`/`docker` | configurable fail-open/fail-closed | Strong path guards + approval gates |

## Gap analysis for DrugClaw

1. Container runtime portability:
DrugClaw sandbox runtime detection and health checks were Docker-specific, which creates friction on setups that standardize on Podman.
...
- Added backend-aware runtime selection/availability helpers in `drugclaw-tools::sandbox`.
