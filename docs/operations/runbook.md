...
- List hooks: `drugclaw hooks list`
- Inspect hook: `drugclaw hooks info <name>`
- Disable bad hook quickly: `drugclaw hooks disable <name>`
...
Use this pattern when you want remote terminal/session capability via an external bridge, without embedding terminal runtime logic into DrugClaw core.
...
3. Start the bridge service separately, then start DrugClaw.
...
If any configured MCP server exposes both `memory_query` and `memory_upsert`, DrugClaw enables MCP-first structured-memory operations:
...
- on MCP call failure, timeout, or invalid/unknown response shape, DrugClaw falls back to built-in SQLite memory automatically
