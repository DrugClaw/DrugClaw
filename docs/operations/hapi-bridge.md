...
This guide describes an indirect integration pattern: run a HAPI bridge as an external MCP sidecar and let DrugClaw consume it through MCP tools.

## Why this pattern

- No deep embedding into `agent_engine`.
- Bridge lifecycle and failure domain stay outside DrugClaw core.
...
2. DrugClaw can reach the endpoint.
...
4. Start bridge first, then start DrugClaw:
