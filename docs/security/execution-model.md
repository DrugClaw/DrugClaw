...
- Backend selection semantics:
  ...
  - `docker`: Docker only
  - `podman`: Podman only
- Enable quickly: `drugclaw setup --enable-sandbox`.
- Verify readiness: `drugclaw doctor sandbox`.
...
- Sandbox mount validation:
  ...
  - optional external mount allowlist (`~/.drugclaw/sandbox-mount-allowlist.txt`)
- File path guard:
  - sensitive path deny list
  - symlink validation on existing path prefix
  - optional external path allowlist (`~/.drugclaw/sandbox-path-allowlist.txt`)
