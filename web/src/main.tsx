...
function readAppearance(): Appearance {
  const saved = localStorage.getItem('drugclaw_appearance')
  return saved === 'light' ? 'light' : 'dark'
}

function saveAppearance(value: Appearance): void {
  localStorage.setItem('drugclaw_appearance', value)
}

function readUiTheme(): UiTheme {
  const saved = localStorage.getItem('drugclaw_ui_theme') as UiTheme | null
  return UI_THEME_OPTIONS.some((t) => t.key === saved) ? (saved as UiTheme) : 'green'
}

function saveUiTheme(value: UiTheme): void {
  localStorage.setItem('drugclaw_ui_theme', value)
}
    ...
    <AssistantRuntimeProvider key={runtimeKey} runtime={runtime}>
      <div className="aui-root h-full min-h-0">
        <Thread
          ...
          strings={{
            composer: {
              input: { placeholder: 'Message DrugClaw...' },
            },
          }}
  ...
  async function submitLogin(password: string): Promise<void> {
    ...
    try {
      ...
      if (e instanceof ApiError) {
        if (e.status === 401) {
          setAuthMessage('Password is incorrect. Please try again or reset with `drugclaw web password-generate`.')
          return
        }
        if (e.status === 429) {
  ...
  async function saveConfigChanges(): Promise<void> {
    try {
      ...
      if (provider === 'openai-codex') {
        ...
        if (apiKey || baseUrl) {
          setSaveStatus('Save failed: openai-codex ignores api_key/llm_base_url in drugclaw config. Configure ~/.codex/auth.json and ~/.codex/config.toml.')
          return
        }
      }
      ...
      setConfigSelfCheck(selfCheck)
      setConfigSelfCheckLoading(false)
      setSaveStatus('Saved. Restart drugclaw to apply changes.')
    } catch (e) {
          ...
          <Dialog.Content maxWidth="520px">
            ...
            <div className="mt-4 space-y-3">
              <ConfigFieldCard
                ...
                description={<>Copy <code>x-bootstrap-token</code> from DrugClaw startup logs.</>}
              >
                <TextField.Root
                  className="mt-2"
        ...
        <Dialog.Root open={configOpen} onOpenChange={setConfigOpen}>
          <Dialog.Content maxWidth="1120px" className="overflow-hidden flex flex-col" style={{ width: "1120px", height: "760px", maxWidth: "1120px", maxHeight: "760px" }}>
            ...
            <Dialog.Description size="2" mb="3">
              Channel-first configuration. Save writes to drugclaw.config.yaml. Restart is required.
            </Dialog.Description>
            {configSelfCheck ? (
              <Callout.Root
                color={
